...
- (void) testDecodeLongTags
{
    NSUInteger pos = 0;
    const unsigned char data1[] = { 0x83, 0x65, 0x00, 0x00 };
    NSData *data = [NSData dataWithBytes:data1 length:sizeof(data1)];
    UMASN1Tag *tag = [[UMASN1Tag alloc]initWithBerData:data atPosition:&pos context:NULL];
    XCTAssert(tag.tagNumber == 3,"Can not decode tag 3");
    pos = 0;
    const unsigned char data2[] = { 0xFF, 0x65, 0x00, 0x00 };
    data = [NSData dataWithBytes:data2 length:sizeof(data2)];
    tag = [[UMASN1Tag alloc]initWithBerData:data atPosition:&pos context:NULL];
    XCTAssert(tag.tagNumber == 101,"Can not decode tag 101");

    pos = 0;
    const unsigned char data3[] = { 0xFF, 0xFF, 0x7F, 0x00 };
    data = [NSData dataWithBytes:data3 length:sizeof(data3)];
    tag = [[UMASN1Tag alloc]initWithBerData:data atPosition:&pos context:NULL];
    XCTAssert(tag.tagNumber == 16383,"Can not decode tag 16383");

}

- (void) testEncodeLongTags
{
    UMASN1Tag *tag = [[UMASN1Tag alloc]init];
    tag.isConstructed = YES;
    tag.tagClass = UMASN1Class_ContextSpecific;
    tag.tagNumber = 3;
    NSData *data = [tag berEncoded];
    const unsigned char *bytes = (const unsigned char *)data.bytes;
    XCTAssert(bytes[0] == 0xA3,@"Can not encode ContextSpecific[3]");
    
    tag.tagNumber = 101;
    data = [tag berEncoded];
    bytes = (const unsigned char *)data.bytes;
    XCTAssert(bytes[0] == 0xBF,@"Can not encode ContextSpecific[101] (byte2)");
    XCTAssert(bytes[1] == 0x65,@"Can not encode ContextSpecific[101] (byte2)");

    tag.tagNumber = 16383;
    data = [tag berEncoded];
    bytes = (const unsigned char *)data.bytes;
    XCTAssert(bytes[0] == 0xBF,@"Can not encode ContextSpecific[16383] (byte1)");
    XCTAssert(bytes[1] == 0xFF,@"Can not encode ContextSpecific[16383] (byte2)");
    XCTAssert(bytes[2] == 0x7F,@"Can not encode ContextSpecific[16383] (byte3)");
